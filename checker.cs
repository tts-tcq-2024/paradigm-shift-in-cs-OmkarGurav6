...
class WarningConfig
{
    ...
    public WarningConfig(float max, float min, bool warningEnabled)
    {
        ...
        WarningTolerance = CalculateWarningTolerance(max, min);
    }

    private float CalculateWarningTolerance(float max, float min)
    {
        return (max - min) * 0.05f; // 5% of the range
    }
}
